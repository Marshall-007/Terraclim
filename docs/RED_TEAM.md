...
### 3.3 "Battle Plan / Season Water Bank / Day Zero" is a strong, less-copyable angle, so lean harder. (**Medium (opportunity)**)
...
**Fix:** Make the backend default to `DEMO_DATE` env when `as_of` is absent (contract says it does, but test it), and add a loud banner "Viewing: 20 Jan 2026 (peak season)" so a slip is obvious, not silent.
...
**Fix:** Pitch seasonal-critical-window value + off-season planning (Water Bank, next-season suitability via TerraClim), not literal 365-day use. Be honest that the value concentrates in the deficit-irrigation window, but that's still a compelling wedge.
